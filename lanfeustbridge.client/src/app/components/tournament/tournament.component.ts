...
export class TournamentComponent implements OnInit {
    ...
    // used to be a `deals` property, but it would return a different array each time, confusing change detection
    _dealIds: {id: number}[] = [];
    ...
    syncDealIds() {
        this._dealIds.length = 0;
        //const result = [];
        for (let i = 0; i < this._tournament.nbDeals; i++)
            this._dealIds.push({ id: i + 1 });
    }
