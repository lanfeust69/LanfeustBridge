...
public class SimpleDealsService : IDealsService
{
    ...
    private Dictionary<int, Deal[]> InitializeDeals()
    {
        ...
        if (File.Exists(_dataFile))
            result = System.Text.Json.JsonSerializer.Deserialize<Dictionary<int, Deal[]>>(File.ReadAllText(_dataFile)) ??
                new Dictionary<int, Deal[]>();
        else
    ...
    {
        File.WriteAllText(_dataFile, System.Text.Json.JsonSerializer.Serialize(Deals));
    }
}
