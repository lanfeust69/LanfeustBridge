...
namespace LanfeustBridge.Models
{
    public class Individual9_18 : IMovement
    {
        ...
        {
            Order = 6,
            Id = typeof(Individual9_18).Name.ToLower(),
            Name = $"Individual for {NB_PLAYERS} players, 18 or 36 deals",
            Description = "Only accepts 18 rounds : 2 rounds playing with each of the 8 other players, plus a bye",
        ...
        public Deal[] CreateDeals(int nbTables, int nbRounds, int nbDealsPerRound)
        {
            ...
            for (int i = 0; i < nbDeals; i++)
                deals[i] = Deal.CreateDeal(i + 1, nbRounds, Deal.ComputeDealer(i % 4 + 1), Deal.ComputeVulnerability(i % 4 + 1));
            return deals;
        }
        ...
        public MovementValidation Validate(int nbTables, int nbRounds)
        {
            ...
            if (nbRounds != 18)
                reasons.Add("Only 18 rounds are allowed for 9-player individuals");
